...
import { CreatorID, CursorBuffer, NetworkID } from "./networking-types";
import { PlayerInfo } from "aframe";
...
export type DeserializeFromStorageFn = (
  eid: EntityID,
  original: StoredComponent
) => void;
...
export function setInitialNetworkedData(
  eid: EntityID,
  nid: NetworkID,
  creator: CreatorID
): void;
export function defineNetworkSchema(component: IComponent): SerDeT;
export function deserializerWithMigrations(
  migrations: Map<number, MigrationFn>,
  apply: ApplyFn
): DeserializeFromStorageFn;
export function read(prop: any, eid: EntityID): any;
export function write(prop: any, eid: EntityID, value: any): void;

export function takeOwnership(world: HubsWorld, eid: EntityID): void;
export function takeSoftOwnership(world: HubsWorld, eid: EntityID): void;
export function getPlayerInfo(sessionId: string): PlayerInfo;
